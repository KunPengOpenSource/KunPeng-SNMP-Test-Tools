...
namespace SnmpSharpNet
{
	...
	public class Oid: AsnType, ICloneable
	{
		...
		public virtual void Set(int[] value)
		{
			...
			if (value.Length < 2)
				throw new ArgumentOutOfRangeException("value", "Minimum 2 sub-oids required for valid Oid");
		...
		/// <summary> Add the specified identifiers to the
		/// current object.
		/// </summary>
		/// <param name="ids">The array of identifiers to Append
		/// </param>
		public virtual void Add(int[] ids)
		{
			if (ids != null && ids.Length != 0)
			{
				int[] tmp = new int[_data.Length + ids.Length];
		...
		{
			int[] tmp = new int[_data.Length + 1];
			Array.Copy(_data, 0, tmp, 0, _data.Length);
			tmp[_data.Length] = id;
			_data = tmp;
		}
		...
		public virtual int Compare(int[] ids)
		{
		...
		/// <param name="ids">Integer array to Compare with</param>
		/// <param name="dist">Number of oid values to compare</param>
		/// <returns>0 if equal, -1 if less then and 1 if greater then.</returns>
		public virtual int Compare(int[] ids, int dist)
		{
		...
		/// </remarks>
		/// <param name="ids">Oid to compare against</param>
		/// <returns>1 if class is greater then argument, -1 if class value is less then argument, 0 if the same</returns>
		public int CompareExact(int[] ids)
		{
			if (ids == null)
				return 1;
		...
		/// <summary> Test for equality. Returns true if 'o' is an instance of
		/// an Oid and is equal to self.
		/// </summary>
		/// <param name="obj">The object to be tested for equality.
		/// </param>
		/// <returns> True if the object is an Oid and is equal
		/// to self. False otherwise.
		/// </returns>
		...
		/// </summary>
		/// <returns>Internal integer array buffer.</returns>
		...
		/// </summary>
		/// <returns>Integer array representing the Oid class IDs</returns>
		public int[] ToArray()
		{
		...
		/// <returns>Oid value or -1 on error</returns>
		public int this[int index]
		{
			get
			{
				if (index < 0 || index >= _data.Length)
					return -1; // Don't throw exceptions. Who would catch it?
				return _data[index];
			}
		}
		...
		protected byte[] encodeInstance(int number)
		{
		...
		public override int decode(byte[] buffer, int offset)
		{
			...
			// save offset before starting decode
			int oldOffset = offset;

			// decode the first byte
			--headerLength;
			int oid = Convert.ToInt32(buffer[offset++]);
			list.Add(oid / 40);
			list.Add(oid % 40);
		...
		private static int[] Parse(System.String oidStr)
		{
