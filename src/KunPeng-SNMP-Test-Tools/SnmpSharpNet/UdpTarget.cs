	...
	public class UdpTarget: UdpTransport
	{
		...
		public SnmpPacket Request(Pdu pdu, IAgentParameters agentParameters)
		{
			...
			if (agentParameters.Version == SnmpVersion.Ver3)
			{
				...
				Console.WriteLine("Encoding packet with engine id {0} boots {1} time {2}",
					packet.USM.EngineId.ToString(), packet.USM.EngineBoots, packet.USM.EngineTime);
			...
			else if (agentParameters.Version == SnmpVersion.Ver3)
			{
				...
				SecureAgentParameters param = (SecureAgentParameters)agentParameters;
				...
				{
					param.UpdateDiscoveryValues(packet);
					return packet;
				}
				else
				{
					if (!param.ValidateIncomingPacket(packet))
					{
						return null;
					}
					else
					{
						param.UpdateDiscoveryValues(packet); // update time, etc. values
						return packet;
					}
				}
			}
		...
		internal void AsyncResponse(AsyncRequestResult result, IPEndPoint peer, byte[] buffer, int buflen)
		{
			...
			else
			{
				...
				{
					SnmpV1Packet packet = new SnmpV1Packet();
					AgentParameters param = (AgentParameters)_agentParameters;
					try
					{
				...
				{
					SnmpV3Packet packet = new SnmpV3Packet();
					SecureAgentParameters param = (SecureAgentParameters)_agentParameters;
					param.InitializePacket(packet);
					try {
						packet.decode(buffer, buflen);
					}
