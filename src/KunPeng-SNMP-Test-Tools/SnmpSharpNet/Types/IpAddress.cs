...
namespace SnmpSharpNet
{
	...
	/// </remarks>
	[Serializable]
	public class IpAddress : OctetString, IComparable<IPAddress>, IComparable<IpAddress>
	{
		...
		public IpAddress(UInt32 inetAddr)
			:this()
		{
			Set(inetAddr);
		}
		...
		/// <returns>0 if contents are the same, -1 if current class is less or 1 if greater then supplied class.</returns>
		public int CompareTo(IPAddress other)
		{
			byte[] b = other.GetAddressBytes();
			if (_data == null)
				return -1;
		...
		{
			IpAddress ipa = obj as IpAddress;
			if (ipa == null)
				return false;
			if (CompareTo(ipa) != 0)
				return false;
